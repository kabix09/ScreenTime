...
/* countriees https://gist.github.com/adhipg/1600028 */
...
/*
	actor			1:n movie characters
	movie character 1:n movies (one move can have few parts) && movie n:1 movie character => n:n (new table)						

	the same role can be played by other person dependents from movie part..... :)
*/
...
CREATE TABLE actor(
	...
	CONSTRAINT FK_Actor_Country FOREIGN KEY(nationality_id) REFERENCES Country(id)
) 
go


-- there can be many characters played by other persons
IF OBJECT_ID(N'dbo.character', N'U') IS NULL
CREATE TABLE character
(
    id int NOT NULL IDENTITY(1,1),		/* is it still necesary? */
	role_name VARCHAR(55) NOT NULL,
	actor_id INT NOT NULL,
	CONSTRAINT PK_Character PRIMARY KEY(id),
	CONSTRAINT UQ_Character_Signature UNIQUE (role_name, actor_id),
...
(
	id INT NOT NULL IDENTITY(1,1),
	title VARCHAR(255) NOT NULL,	-- full nime: main title + subtitle
	--subtitle VARCHAR(255),		-- optional
	production_year DATETIME2 NOT NULL,
	duration_time INT NOT NULL,
	world_premiere_date DATETIME2 NOT NULL,
	CONSTRAINT PK_Moivie PRIMARY KEY (id),
	CONSTRAINT UQ_Movie_Signature UNIQUE(title, production_year)
)
go
...
--title1 - category A
--title1 - category B
-- add new genre on AdminPanel

IF OBJECT_ID(N'dbo.movie_character', N'U') IS NULL
CREATE TABLE movie_character
(
	movie_id INT NOT NULL,
	character_id INT NOT NULL,
	time_on_scene INT NOT NULL,
	CONSTRAINT PK_MovieCharacter PRIMARY KEY (movie_id, character_id),
	CONSTRAINT FK_MC_Movie FOREIGN KEY (movie_id) REFERENCES Movie(id),
	CONSTRAINT FK_MC_Character FOREIGN KEY (character_id) REFERENCES Character(id)
)
go
